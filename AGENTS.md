...
- LuCI ACL is **role-locked**: admin+LuCI → **Full** (`*`); readonly+LuCI → **diagnostic** (Status Overview/Routing/Realtime, Network Interfaces/Diagnostics, Device health, view-only User Management — stock groups on `list read` only, plus narrow `luci-app-usrmanage-diagnostic-rpc` for page RPCs; no `luci-base` / `getWirelessDevices`). No `--scope` picker.
...
- Review SoT: [docs/security-review.md](docs/security-review.md) (proof class `host` | `lab` | `manual`). Audit how (multi-model VVAH-style): [`.cursor/skills/security-audit/SKILL.md`](.cursor/skills/security-audit/SKILL.md). Do not reopen accepted residuals / #3 won't-fix without new evidence. Feature PRs that touch auth/session/password/rpcd/ACL/signing update the ledger in the same PR; `lab`-class locks need qemu-smoke or a release-blocking issue (DRY_RUN/host stubs are not proof). Findings: `security` label + IDs (`S1`, `R1`, `P1`).
...
Full protocol: [docs/developer/coderabbit.md](docs/developer/coderabbit.md).
